...
def build_dictionary_by_length(L, forbidden_words=None, forbidden_prefixes=None):
    """
    Build admissible dictionary with abecedary structure and forbidden prefix filtering.

    Advanced case (Abecedary):
      1. Apply abecedary structure: limit words per length
         - l = 1  -> keep all 2 words
         - l >= 2 -> keep 3 * 2^(l-2) words
      2. Apply forbidden prefix filtering
         - forbidden_prefixes = ("11",)
         - words starting with 11 are forbidden
    """
    ...
    for length in range(1, L + 1):
        ...
        # Step 1: Apply abecedary structure constraint
        if length == 1:
            kept_words = words[:]
        else:
            n_keep = 3 * (2 ** (length - 2))
        ...
        # Step 2: Apply forbidden prefix constraint
...
def run_qubo_experiment():
    ...
    print("=" * 80)
    print(f"QUBO SOLVER — ADVANCED CASE (ABECEDARY) (n = {N}, L = {L})")
    print("=" * 80)
    print(f"  Case label:          {CASE_LABEL}")
    print(f"  n:                   {N}")
    print(f"  L:                   {L}")
    print(f"  Dictionary:          Abecedary structure")
